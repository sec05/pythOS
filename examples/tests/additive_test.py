...
result = ark.ark_solve([f1,f2], 0.1, y0, 0, tf, [tableau_3_e,tableau_3_i], None, fname='ark_solve_no_labels.csv')
output(verbose, 0.02, result, solution, "Additive RK without labels")

result = ark.ark_solve(master_function, 0.1, y0, 0, tf, [tableau_3_e,tableau_3_i], labels, fname='ark_solve_with_labels.csv')
output(verbose, 0.02, result, solution, "Additive RK with labels")
...
result = ark.ark_solve([f1,f2], 0.2, y0, 0, tf, [tableau_e,tableau_i], None, fname='ark_solve_adaptive.csv',rtol=1e-6,atol=1e-8)
output(verbose, 0.02, result, solution, "Adaptive additive RK without labels")

result = ark.ark_solve(master_function, 0.2, y0, 0, tf, [tableau_e,tableau_i], labels, fname='ark_solve_adaptive_no_labels.csv',rtol=1e-6,atol=1e-8)
output(verbose, 0.02, result, solution, "Adaptive additive RK with labels")
...
A = [[Aee, Aei], [Aie, Aii]]
b = [bE, bI]
result = gark.gark_solve([f1,f2],0.1,y0,0,tf,A,b, None, fname='gark_solve_no_labels.csv')
output(verbose,0.1,result,solution, "Generalized additive RK without labels")

result = gark.gark_solve(master_function, 0.1, y0, 0, tf, A, b, labels, fname='gark_solve_with_labels.csv')
output(verbose, 0.1, result, solution, "Generalized additive RK with labels")
