...
- master_function: the operators to use
		 
	These must be either a list of functions in the order you wish to use them, or a callable master function.
	
	If a list then each function each must be callable with arguments (t, y) and return a numpy array, unless using the finite element capabilities.
	
	If a master function is passed then it must be callable with arguments (t,y,label), where each label is a member of the function labels passed. Ths gets converted to a list of operators each distinguished by a call to master_function by a different label.
...
- function_labels
	...
	Defaults to None if nothing is passed. Otherwise, if master_function is callable then each label is used to generate the list of operators. 

	Labels can be grouped as (l1, l2,...) then the oeprator generated by this label is O(t,y) = master_function(t,y,l1) + master_function(t,y,l2) + ...
...
- ivp\_methods: a dictionary to control the selection of the ADAPTIVE solver. 
	 ...
	 `IDA`, or any of the method strings for MRIStep, ERKStep or ARKStep.
		 ...
		 If a master function is given the labels must be of the 
		 form ((implicit labels), (explicit labels)),
		 this is so the code can form the correct implicit and explicit operators.
		 ...
		 (t, y, dydt) and return a residual. Likewise, if using labels the 
...
- function_labels
	...
	Labels can be grouped as (op, l1, l2,...) where op = "sum" or "prod", if not provided then it defaults to "sum". Then the oeprator generated by this label is of the form op(master_function(t,y,l1), master_function(t,y,l2),...)
- fname: a filename to save intermediate results to.  
...
- fs
	...
	The slow operator. Either a callable function with arguments (t,y) and return a numpy array, unless using finite element capabilities then it should be a Form, or a label to be passed to the master_function.
	
- ff
	
	The fast operator. Either a callable function with arguments (t,y) and return a numpy array, unless using finite element capabilities then it should be a form, or a label to be passed to the master_function.

Note a lablel can be a value, or a tuple of values. If it is a tuple it must be of the form (l1, l2,...) so that the oeprator generated by this label is of the form O(t,y) = master_function(t,y,l1) + master_function(t,y,l2) + ...

Optional arguments:
- master_function: a callable function with arguments (t,y,label) and returns a numpy array.

	If either fs or ff is a label  they are redefined as calls to master_function with their value as the label.
- fname: a filename to save intermediate results to.  
...
- fi
	...
	The slow operator.  If the method is IMEX at the slow scale, this is the implicit slow operator. Either a callable function with arguments (t,y) and return a numpy array, unless using finite element capabilities then it should be a Form, or a label to be passed to the master_function.
...
- fe (optional)
	...
	The explicit slow operator for methods that are IMEX at the slow scale. Either a callable function with arguments (t,y) and return a numpy array, unless using finite element capabilities then it should be a Form, or a label to be passed to the master_function.
...
- master_function: a callable function with arguments (t,y,label) and returns a numpy array.
	...
	If either fi, ff, or fe is a label they are redefined as calls to master_function with their value as the label.
- fname: a filename to save intermediate results to.  
